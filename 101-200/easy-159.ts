interface IMoves {
	[playerMove: string]: {
		[computerMove: string]: string
	}
}

const rpsls = (playerMove: string): string => {
	const options: IMoves = {
		rock: {
			rock: 'Tie.',
	...
	const computersMove = computerChoices[Math.floor(Math.random() * computerChoices.length)];
