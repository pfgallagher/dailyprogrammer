...
const inc = (arr: number[]): number[] => arr.map(el => el + 1);

const zeroArr = (len: number): number[] => Array(len).fill(0);

const funnyPlant = (numPeople: number, numStartFruit: number): number => {
	let numWeeks = 1;
	let fruit = zeroArr(numStartFruit);
...
// 5
console.log(funnyPlant(50_000, 1));
// 14
console.log(funnyPlant(150_000, 250));
// 9
