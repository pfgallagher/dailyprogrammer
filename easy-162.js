...
var capitalizationRules = function (word) {
    if (word.length > 1) {
        var exclamationI = word.indexOf("!");
        var caretI = word.indexOf("^");
        if (exclamationI >= 0) {
...
var decompress = function (dictionary, compressedText) {
    var rules = compressedText
        .replace(/(\d*)/g, function (match) {
        if (match) {
            var iFromNum = parseInt(match, 10);
            return dictionary[iFromNum];
        }
        return match;
    })
        .split(" ")
        .map(function (el) { return capitalizationRules(el); })
        .filter(function (el) { return el !== "e" && el !== "E"; })
...
console.log(decompress(["is", "my", "hello", "name", "stan"], "2! ! R 1^ 3 0 4^ . E"));
// HELLO!
// My name is Stan.
var seussDictionary = [
    "i",
