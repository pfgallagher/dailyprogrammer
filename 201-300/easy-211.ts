const nameGame = (name: string): string => {
	const baseName = /[aeiou]/i.test(name[0])
		? name.toLowerCase()
		: name.slice(1);
	const bName = name.startsWith("B") ? `Bo-${name.slice(1)}` : `B${baseName}`;
	const fName = name.startsWith("F") ? `Fo-${name.slice(1)}` : `F${baseName}`;
	const mName = name.startsWith("M") ? `Mo-${name.slice(1)}` : `M${baseName}`;
